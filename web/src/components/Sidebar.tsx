...
const Sidebar = async () => {
  ...
  return (
    <aside className="w-full lg:pl-8 space-y-8 animate-fade-in-up">
            ...
            return (
              <li key={article.slug}>
                <Link href={`/blog/${article.slug}`} className="flex items-start space-x-3 group">
                  ...
                  <span className="text-left font-semibold text-dark-gray/90 group-hover:text-seafoam-green transition-colors duration-300 break-words">
                    {article.title ?? '無題の記事'}
                  </span>
                </Link>
              </li>
